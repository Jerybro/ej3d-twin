# EJ Process Intelligence 平台

穎杰科技製程智能平台：**數據與物理雙驅動——先成就數據孿生，再映射視覺孿生；
物理約束治理 AI 外插，每個模型附驗證度。**
...
|---|---|---|
| `/` | 平台首頁 | 產品導航 |
| `/data` | **產品2：資料前處理** | 上傳 CSV/Excel、canvas 框選圈選剔除、規則式清洗（五級穩態過濾同源，逐筆剔除原因可審計）、皮爾森相關熱圖、匯出；**CoolProp 物性模組**：物理合理性檢查（標記超出物性邊界的數據＝AI 外插柵欄）＋物性欄位推算 |
| `/twin` | **產品3：孿生檢視** | 3D 場景＋即時數據熱力圖＋工安情境＋製程計算（原 DEMO 全功能；`?scene=` 載自建場景） |
| `/twin/editor` | **產品3：3D 模塊編輯器** | 9 類工業素材（反應器/蒸餾塔/儲槽/閥/泵/熱交/加熱爐/偵測器/建物）放置、移動/旋轉/縮放（W/E/R）、屬性編輯、管線繪製（Enter 完成）、場景存檔——存檔格式即 plant.json schema，孿生檢視原生相容 |
| `/twin/pid` | **產品3：P&ID 管理** | 圖面上傳/檢視；保留 4K 影片→3D 重建佔位 |

場景系統：`data/scenes/*.json`＋`/api/scenes` CRUD；共用建模模組
`static/js/plant-builders.js`（簡易/精細雙版本幾何）。
...
## 分析層：Tatoray 製程灰箱代理模型（真實廠數據驗證）

以真實 Tatoray 轉烷化單元（TA-32）**977 天品質日報**訓練驗證的代理模型
（`server/surrogate.py`＋`models/tatoray_surrogate.joblib`，305KB）：

- **輸入** S601 進料組成（TOL/TMB/MEB/C9…15 項）→ **輸出** S604 出口組成（BZ/TOL/X/C9 wt%）
- 灰箱結構：L1 物理特徵（甲基/苯環比、歧化 2TOL→BZ+X 與轉烷 TOL+TMB→2X 驅動力）
  ＋ L2 GBDT 學「變化量 Δ」（以製程慣性為底，soft-sensor 標準式，保證不輸天真基準）
- **時序切分驗證（後 20% 未見時段）**：R² 0.79–0.88、MAE 0.02–0.79 wt%（相對誤差 ~1.5–2%）
- UI「製程計算」chip：What-if 滑桿調進料組成 → 即時計算出口，與最近實測對照
- 已知天花板：品質日報無 DCS 操作條件——補 R611 WABT／進料流量／H₂/HC 比
  三個 tag 後可升級為全 what-if 反應模型（此即 OPC UA 介接的價值閉環）
- **NDA 隔離**：原始數據不進版控（.gitignore 強制）、API 只出模型計算值；
  models/ 內僅權重與單點基準，無可回溯序列
